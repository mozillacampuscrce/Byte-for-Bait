...
(function(){
	...
	var $screenshoot_slider1  = $('#screenshoot_slider1');

	if ( $screenshoot_slider1.length > 0 ) {
		$screenshoot_slider1.owlCarousel({
		    stopOnHover : true,
			slideSpeed : 800,
			autoHeight : true,
			itemsCustom: [
		...
		screenshoot_slider_obj.slider1 = $screenshoot_slider1.data('owlCarousel');

		$('.screenshoot-arrow1').click(function(e){

			e.preventDefault();

			if ( $(this).hasClass('left-arrow') ) {
				screenshoot_slider_obj.slider1.prev();
			} else {
				screenshoot_slider_obj.slider1.next();
			}
		});
	}
}());
...
(function(){
	...
	if ( $('#whySlider1:visible').length > 0 ) {
		var sliderWrap = $('#whySlider1:visible'),
			slideItems = sliderWrap.children(),
			height = slideItems.getMaxHeight(),
			lists = $('#why-feature-lists li'),
			current = 0;
...
if ( $mapWrapper.length > 0 ) {
	var map = new GMaps({
		div: '#map',
		lat : 19.021122, // Provide your longtitude
		lng : 72.8411113,  // Provide your longtitude
		scrollwheel: false,
